...
- 失败阶段诊断显示 256 次中只有 5 次曾达到 0.7m 高度、3 次曾达到 30°以内倾角，且只有
  ...
  代表真实起身能力。
- v4 继续完成 iteration 201--500，全部 500 条 metrics 有限，最终 std `0.20916`、KL
  `0.01680`、动作裁剪为零。iteration 500 标准静止起身首次出现 `1/256` 物理成功，来自
  俯卧组；9 次曾达到 0.7m、4 次曾扶正、3 次瞬时同时满足，最大高度 p90 从 iteration 200
  的 `0.588m` 升至 `0.601m`，但中位数仍约 `0.350m`。仰卧及左右侧卧仍全部失败。
- iteration 201--300 保持 fall-switch probability 为 0，最后 20 轮 Recovery reward 均值
  `0.02657`；iteration 301--400 ramp 后为 `0.01771`，401--500 完全启用后为 `0.01854`。
...
## iteration-1000 Recovery reset 分层诊断（2026-08-11）

- 用同一个 iteration-1000 checkpoint、256 个环境、1000 steps、无外力，分别从 Recovery
  轨迹的前/中/后三分之一 reset；三组保留训练期 reset 噪声和参考速度。另以最低倒地帧、
  零速度、无噪声的 standard recovery 作为真正静止倒地基线。
- 严格物理成功（高度 `>=0.7m`、倾角 `<=30deg` 持续 `0.5s`）分别为：最低倒地点
  `0/256`、前段 `1/256`（`0.39%`）、中段 `10/256`（`3.91%`）、后段 `75/256`
  （`29.30%`）。terminal-reference success 分别为 `0/256`、`0/256`、`1/256`、
  `17/256`。
- 曾同时达到直立阈值但未必保持 0.5 秒的 trial 分别为 `2/256`、`12/256`、`79/256`、
  `241/256`；最大高度中位数依次为 `0.341m`、`0.428m`、`0.671m`、`0.769m`。
- 后段的 256 个初态中有 232 个已归入 `other`，即不再是明确的仰卧、俯卧或侧卧；75 个
  物理成功全部来自这组。明确躺姿在后段仍为 `0/24` 物理成功。中段 10 个成功中 9 个也
  来自 `other`，只有俯卧为 `1/64`；仰卧和左右侧卧均为 0。
- 结论：训练内 success 的主要来源是 Recovery 轨迹中后段、已经抬高或接近直立的状态，
  不能证明策略掌握了从地面开始的撑地、翻身和抬高。训练量继续增加只是在优化较容易的
  后半程，尚未解决前半程探索/监督覆盖问题。
